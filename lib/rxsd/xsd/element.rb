  ...
  module XSD
    ...
    class Element
      ...
      def self.from_xml(node)
        ...
        element.nillable  = node.attrs.key?('nillable') ? node.attrs['nillable'].to_b : false
        element.abstract  = node.attrs.key?('abstract') ? node.attrs['abstract'].to_b : false

        if node.parent.name == Schema.tag_name
          element.form = node.parent.attrs['elementFormDefault']
        else
          # FIXME: ignoring reference namepsace prefix (if any) for now
          ref = node.attrs['ref']
          ref = ref.split(':')[1] unless ref.nil? || ref.index(':').nil?
          element.ref = ref

          element.substitutionGroup = node.attrs['substitutionGroup']
          element.form = node.attrs.key?('form') ?
                                        node.attrs['form'] :
                                         node.root.attrs['elementFormDefault']

          element.maxOccurs  = node.attrs.key?('maxOccurs') ?
                                   (node.attrs['maxOccurs'] == 'unbounded' ? 'unbounded' : node.attrs['maxOccurs'].to_i) : 1
          element.minOccurs  = node.attrs.key?('minOccurs') ?
      ...
      def resolve(node_objs)
        ...
        @ref = node_objs[Element].find { |no| no.name == @ref } unless @ref.nil?
      ...
      def child_attributes
        if !@ref.nil?
          @ref.child_attributes
        elsif [SimpleType, ComplexType].include? @type.class
          @type.child_attributes
        elsif !@simple_type.nil?
          @simple_type.child_attributes
        elsif !@complex_type.nil?
          @complex_type.child_attributes
        end
      end
      end
    end # module XSD
